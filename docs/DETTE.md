...
| 21/07 | **Pas de clé Google Maps** → RouteService replie sur une ligne droite (2 points) + durée à 25 km/h, loggé en warning | Même cause racine que Blaze : la facturation Google. Le repli permet de développer J6-J9 sans clé ; les factories ont de vraies polylines pour tester le matching. | **Avant le J7 idéalement** (Places Autocomplete en a besoin) : activer la facturation, créer la clé (Routes + Places, restreinte), la mettre dans GOOGLE_MAPS_API_KEY. Même session que Blaze. |
